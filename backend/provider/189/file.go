...
func (f File) UploadCreate(md5Hash hash.Hash) (fileId string, err error) {
	...
	fileInfo, err := f.FS.GetFileInfo(f.Ctx, d, f.File.ProviderFolder)
	var fileMd5 string
	if fileSize == 0 {
...
func (f File) WriteTo(w io.Writer) (n int64, err error) {
	...
	srcFileInfo, err := f.FS.GetFileInfo(f.Ctx, f.File.Name, f.File.ProviderFolder)
	dstFileInfo, err := f.FS.GetFileInfo(f.Ctx, dstD, f.File.ProviderFolder)
