...
func (a API) queryMeta(folderId string) (FileListAO, error) {
	...
	req.SetResult(&ret)
	_, err := req.Get(fmt.Sprintf("https://cloud.189.cn/api/open/file/listFiles.action?noCache=0.7362081385378736&pageSize=60&pageNum=1&mediaType=0&folderId=%s&iconOption=5&orderBy=lastOpTime&descending=true", folderId))
	if ret.ResMsg != "成功" {
